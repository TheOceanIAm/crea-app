...
function parseCompaniesPayload(raw: unknown): { ok: boolean; companies: CompanyRow[] } {
  if (!raw || typeof raw !== 'object') return { ok: false, companies: [] }
  const o = raw as Record<string, unknown>
  const arr = Array.isArray(o.companies) ? o.companies : []
  return {
    ok: o.ok === true,
    companies: arr.map((r) => {
...
export default function CeoCompaniesScreen() {
  ...
  const load = useCallback(async (isRefresh?: boolean) => {
    ...
    setHint(null)
    const { data, error } = await supabase.rpc('ceo_list_companies', {
      p_search: debounced,
      p_limit: 150,
    })
    setLoading(false)
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <Text style={styles.empty}>No companies match this search.</Text>
          }
          renderItem={({ item }) => {
