-- Booking DM accept (availability invite): add freelancer to project_members so Crew tab shows them.
-- Marketplace job_application trigger only runs on applications — booking uses CREA_BOOKING_JSON_V1 messages only.
...
comment on function public.sync_project_member_from_booking_accept(uuid) is
  'After freelancer accepts a CREA_BOOKING_JSON_V1 DM: add them to project_members (same rules as job_application accept trigger).';

revoke all on function public.sync_project_member_from_booking_accept(uuid) from public;
grant execute on function public.sync_project_member_from_booking_accept(uuid) to authenticated;
